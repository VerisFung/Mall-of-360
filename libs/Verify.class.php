...
class Verify{
    ...
    private $numSet='1234567890';// 数字验证码
    private $enSet='abcdefhijkmnpqrstuvwxyzABCDEFGHJKLMNPQRTUVWXY';// 英文验证码
    private $mixSet='2345678abcdefhijkmnpqrstuvwxyzABCDEFGHJKLMNPQRTUVWXY';// 混合验证码
     ...
     * @param integer $mode   模式         [0数字 1英文 2混合 3中文]
     * @param boolean $case   不区分大小写,作用于含有英文的验证码[true不区分 false区分]
     */
    public function __construct($width=135,$height=50,$length=4,$mode=0,$case=true){
        $this->width=$width;
        $this->height=$height;
        $this->img=imagecreatetruecolor($width,$height);
        imagefill($this->img,0,0,$this->randColor(1));
        $this->noisePoint(1000);
        $charWidth=$width/$length;
        $chars='';
        switch($mode){
            case 0: //数字验证码
                for($i=0;$i<$length;$i++){
                    $fontfile=APP_PATH.'libs/Verify/ttfs/'.mt_rand(1,6).'.ttf';
                    $char=$this->numSet[mt_rand(0,strlen($this->numSet)-1)];
                    $chars.=$char;
                    imagettftext($this->img,$height/2,mt_rand(-35,35),$i*$charWidth+10,mt_rand(15,$height-5),$this->randColor(2),$fontfile,$char);
                }
            ...
            case 1: //英文验证码
                for($i=0;$i<$length;$i++){
                    $fontfile=APP_PATH.'libs/Verify/ttfs/'.mt_rand(1,6).'.ttf';
                    $char=$this->enSet[mt_rand(0,strlen($this->enSet)-1)];
                    $chars.=$char;
                    imagettftext($this->img,$height/2,mt_rand(-35,35),$i*$charWidth+10,mt_rand(15,$height-5),$this->randColor(2),$fontfile,$char);
                }
                $_SESSION['verify']=$case?strtolower($chars):$chars; //是否不区分大小写
                break;
            case 2: //混合验证码
                for($i=0;$i<$length;$i++){
                    $fontfile=APP_PATH.'libs/Verify/ttfs/'.mt_rand(1,6).'.ttf';
                    $char=$this->mixSet[mt_rand(0,strlen($this->mixSet)-1)];
                    $chars.=$char;
                    imagettftext($this->img,$height/2,mt_rand(-35,35),$i*$charWidth+10,mt_rand(15,$height-5),$this->randColor(2),$fontfile,$char);
                }
                $_SESSION['verify']=$case?strtolower($chars):$chars; //是否不区分大小写
            ...
            case 3: //中文验证码
                for($i=0;$i<$length;$i++){
                    $fontfile=APP_PATH.'libs/Verify/zhttfs/'.mt_rand(1,4).'.ttf';
                    $char=mb_substr($this->zhSet,mt_rand(0,mb_strlen($this->zhSet,'utf-8')-1),1,'utf-8');
                    $chars.=$char;
                    imagettftext($this->img,20,mt_rand(-35,35),$i*$charWidth+10,mt_rand(20,$height),$this->randColor(2),$fontfile,$char);
                }
                $_SESSION['verify']=$chars;
                break;
        }
    ...
    /**
     * 随机颜色
     * @param  integer $mode 模式 0常规 1暗色 2亮色 3中间色
     * @return resource      随机颜色资源
     */
    private function randColor($mode=0){
        switch($mode){
            case 0:
                $r=mt_rand(0,255);
                $g=mt_rand(0,255);
     ...
     * @return null
     */
    private function noiseLine($n){
        for($i=0;$i<$n;$i++){
            imageline($this->img,mt_rand(0,$this->width),mt_rand(0,$this->height),mt_rand(0,$this->width),mt_rand(0,$this->height),$this->randColor(1));
        }
    }
     ...
     * @return null
     */
    private function noisePoint($n){
        for($i=0;$i<$n;$i++){
            imagesetpixel($this->img,mt_rand(0,$this->width),mt_rand(0,$this->height),$this->randColor(1));
        }
